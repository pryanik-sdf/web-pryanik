...
sudo cp pryanikweb.service "$SERVICE_FILE"

# Replace path in service file
PROJECT_PATH="$(pwd)"
sudo sed -i "s|/path/to/webutility|$PROJECT_PATH|g" "$SERVICE_FILE"
...
if [ ! -f "$SSL_KEY" ] || [ ! -f "$SSL_CERT" ]; then
  ...
  sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout "$SSL_KEY" -out "$SSL_CERT" -subj "/C=RU/ST=Russia/L=Moscow/O=WebUtility/CN=localhost"
...
sudo sh -c "echo 'Environment=SSL_KEY=$SSL_KEY SSL_CERT=$SSL_CERT' >> '$SERVICE_FILE'"
sudo sed -i "s|ExecStart=.*/node /path/to/webutility/index.js|ExecStart=/usr/bin/node /path/to/webutility/index.js|" "$SERVICE_FILE"
